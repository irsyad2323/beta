                       ...
                       ?>
						 ...
						 <div class="form-row">                           
							...
							<div class="form-group col-md-2">
                                <label for="rang">ID USER <?php echo $kota ?></label>                
...
WHERE g.kode_user = r.username and datediff(CURDATE(), r.expiration) > 30 AND g.status_log = 'y' AND r.`status` = 'off' and g.kd_layanan like '".$kota."' ORDER BY jumday DESC;");
				  ...
				  <div class="form-group col-md-4">
                    ...
                    <select class="form-control" type="text" id="status_log" name="status_log" autocomplete="off" >
                    <option></option>
                    <option>y</option>
                    <option>n</option>                 
					...
					<?php 
                        if ($_SESSION["level_user"] != "ikr" && $_SESSION["level_user"] != "psg_dcp" && $_SESSION["level_user"] != "kapten"){
                       ?>
...
WHERE g.kode_user = r.username and datediff(CURDATE(), r.expiration) > 30 AND g.status_log = 'y' AND r.`status` = 'off' and g.kd_layanan like '".$kota."' ORDER BY jumday DESC;");
                    ...
                    <option> <?php
											  $conn = mysqli_connect("117.103.69.22", "kocak", "gaming", "billkapten");
											  $sql_user = mysqli_query($conn, "SELECT * FROM tbl_pegawai WHERE status_pegawai='aktiv' and `status`='PKL';");
											  //$data_user = mysqli_fetch_array($sql_user);
											  //print_r($data_user);
											 while ($data_user = mysqli_fetch_array($sql_user)) {
												echo '<option value"'.$data_user['nama_panggilan'].'">'.$data_user['nama_panggilan'].' </option>';
				  ...
				  <div class="form-group col-md-4">
					...
					<?php 
                        if ($_SESSION["level_user"] != "ikr" && $_SESSION["level_user"] != "kapten"){
                       ?>
         ...
         $(document).ready(function() {
            ...
            var table = $('#tabel_pengguna').DataTable({
                ...
                "ajax": {
                    ...
                    "url":"../models/datapengguna_disable_100hari.php",
...
//alert (a);
    $.ajax({
        url: "../create/create_user_dismantle.php",
