						...
						<div class="container-fluid">
						  ...
						  <div class="card shadow mb-4">
							<div class="card-header py-3">
							  <h6 class="m-0 font-weight-bold text-primary">Data MGM</h6>							  
							 <a href="../controller/export_mgm.php" class="btn btn-success btn-icon-split">
                                        <span class="text">Export DATA MGM</span>
                                    </a>
							</div>
							<div class="card-body">
								...
								<table class="table table-bordered" id="tabel_pengguna" width="100%" cellspacing="0">
								  ...
								  <tbody> 
									<?php 
										  ...
										  // Cek apakah nilainya 'pas'
if ($kota_in == "pas") {
    $kota = "'pas','pas1'";
} else {
    $kota = "'" . $kota_in . "'";
}
										  $table = mysqli_query($koneksi,"SELECT a.key_fal,a.adm_tgl_nominal, b.an_rek, a.tgl_ins_datetime, a.status_wo, a.username_fal, a.nominal_mgm ,b.nama_sobat, b.no_rekening, b.metode_bayar , b.no_wa_sobat, b.an_rek, b.alamat
FROM tbl_fal a 
JOIN tbl_sobat_mgm b
ON a.nama_sobat COLLATE utf8mb4_unicode_ci = b.nama_sobat COLLATE utf8mb4_unicode_ci
WHERE a.kd_layanan in ($kota) and a.status_wo='Sudah Terpasang' 
and a.mgm = 'mgm' and a.verified_mgm = 'n' GROUP BY a.key_fal;");
										  ...
										  <tr id=<?php echo $row['key_fal']; ?>>
											...
											<td data-target="tgl_ins_datetime"> <?php echo $row['tgl_ins_datetime'];?></td>
											...
											class="btn btn-primary btn-sm mr-2 aprove">APPROVED</button>									
					   ...
					   <!-- Begin Page Content -->
						<div class="container-fluid">
						  ...
						  <div class="card shadow mb-4">
							<div class="card-header py-3">
							  <h6 class="m-0 font-weight-bold text-primary">Data APPROVED</h6>
							</div>
							<div class="card-body">
							  <div class="table-responsive">
							  <form name="frm-example" id="frm-example">
    ...
    $kota = "'" . $kota_in . "'";
										  ...
										  $table = mysqli_query($koneksi,"SELECT a.verified_fls, a.ket_mgm, a.key_fal, b.nama_sobat, b.no_rekening, b.metode_bayar , b.no_wa_sobat, b.an_rek, b.alamat , COUNT(a.nama_sobat) as total 
FROM tbl_fal a
JOIN tbl_sobat_mgm b
ON a.nama_sobat COLLATE utf8mb4_unicode_ci = b.nama_sobat COLLATE utf8mb4_unicode_ci
WHERE verif_nominal='n' 
AND verified_mgm='y' 
AND kd_layanan in ($kota)
AND status_wo='Sudah Terpasang' 
AND mgm = 'mgm' 
GROUP BY b.nama_sobat;");
										  $i=1;
										  $no=1;
										  while ($row = mysqli_fetch_assoc($table)){ 
											if( $row['verified_fls'] = 'n'){
												$row['stts_fls'] = '<small class="badge badge-danger">Proses Approved</small>';
											}elseif( $row['verified_fls'] = 'y'){
												$row['stts_fls'] = '<small class="badge badge-success">Approved</small>';
											}
										  ...
										  <tr id=<?php echo $row['key_fal']; ?>>
											...
											<td data-target="hasil"> <?php echo $row['hasil'];?></td>
											<td data-target="stts_fls"> <?php echo $row['stts_fls'];?></td>
											...
											ket_fls="<?php echo $row['ket_mgm'];?>"
											class="btn btn-warning btn-sm mr-2 nominaledit">INPUT NOMINAL</button>									
			  ...
			  <!--h3 class="box-title pull-right">Data Table Customer</h3-->
			   <div class="col-sm-5 pull-right">
				...
				<div class="input-group">
                  <input type="text" class="form-control" id="pilih_bln_thn">               
                    <span class="input-group-btn">
                      <button type="button" class="btn btn-danger filter_bln_thn btn-flat pull-left">Go!</button>
                    </span>
				 </div>
			  </div>
            </div>
					   ...
					   <!-- Begin Page Content -->
						<div class="container-fluid">
						  ...
						  <!-- div class="card shadow mb-4">
							<div class="card-header py-3">
							  <h6 class="m-0 font-weight-bold text-primary">tes keuangan</h6>
							</div>
							<div class="card-body">
							  <div class="table-responsive">
							  <form name="frm-example" id="frm-example">
								<table class="table table-bordered" id="tabel_pengguna3" width="100%" cellspacing="0">
								  ...
								  <tbody> 
									<?php 
										  include('../controller/controller_mysqli.php');
										  $acces_user_log = $_SESSION["username"];
										  $table = mysqli_query($koneksi,"SELECT *, nama_sobat, metode_pembayaran ,no_rekening, COUNT(*) as total 
										  FROM tbl_fal WHERE verif_nominal='y' and verified_mgm='y' and verified_fls='n' and kd_layanan like '".$kota."' and status_wo='Sudah Terpasang' and mgm = 'mgm' GROUP BY nama_sobat;");
										  $i=1;
										  $no=1;
										  while ($row = mysqli_fetch_assoc($table)){ 
										  ...
										  <tr id=<?php echo $row['key_fal']; ?>>
											...
											class="btn btn-info btn-sm mr-2 detail">ACTION</button>									
                 ...
                 <div class="modal-body">
                    <form role="form" method="post" id="formdatapengguna">
						...
						<div class="form-row">
								...
								<select class="form-control" type="text" id="verif_nominal" name="verif_nominal" autocomplete="off" required>								
								<option></option>					
...
/* $(document).ready(function() {
	...
	$.ajax
		({
			type: "Post",
			url: "control/tahun_bulan_sales.php",
			data: value,
			dataType: 'JSON',				  
			...
			$('#detail').DataTable().clear().destroy();
			$('#detail').DataTable({					  
				  "paging": true,
				  "lengthChange": true,
				  "searching": true,
                  ...
                  'responsive': true,
			"ajax": {
				  "url":"../models/datapengguna_salesmgm.php",
				  "type": "POST",					  
				  "data" : function(d){						  
							  d.mulai= start_tgl;
							  d.akhir= end_tgl;					  
