                    ...
                    <?php 
                       ...
                       ?>
						...
						$acces_user_log = $_SESSION["username"];
						  $table = mysqli_query($koneksi,"SELECT a.*, b.status_log as status_gun FROM tbl_disable_noconfirm a, tb_gundala b WHERE a.username_fal = b.kode_user and a.kd_layanan like '".$kota."' and a.kategori_pelanggan='Modem Diambil' ORDER BY a.key_fal DESC");
						  ...
						  <tr id=<?php echo $row['username_fal']; ?>">
							...
							<td data-target="pic"> <?php echo $row['pic_ikr'];?></td>
							<td data-target="kategori_pelanggan"> <?php echo $row['kategori_pelanggan'];?></td>
							<td data-target="tgl_progres"> <?php echo $row['tgl_progres'];?></td>
							<td data-target="off_tgl"> <?php echo $row['off_tgl'];?></td>
							<td data-target="off_total"> <?php echo $row['off_total'];?></td>
							<td data-target="status_gun"> <?php echo $row['status_gun'];?></td>
							...
							kategori_pelanggan="<?php echo $row['kategori_pelanggan'];?>"
							lain_lain="<?php echo $row['lain_lain'];?>"
							status_log="<?php echo $row['status_log'];?>"
							class="btn btn-info btn-sm mr-2 editberhenti">Edit</button>
            ...
            <div class="modal-content">
                 ...
                 <div class="modal-body">
					...
					<div class="form-group col-md-4">
                                ...
                                <input class="form-control" type="text" id="username_berhenti" name="username_sinden" autocomplete="off"  disabled>
                            </div>
					
						<div class="form-group col-md-6">
                            <label for="lname">Nama</label>
                            <input class="form-control" type="text" id="nama_berhenti" name="nama_sinden" placeholder="alamat..." autocomplete="off" disabled>
                        </div> 
					

					<div class="form-group col-md-4">
                    <label for="rang">Kategori Disable</label>                
                    <select class="form-control" type="text" id="kategori_pelanggan_berhenti" name="kategori_pelanggan_berhenti" autocomplete="off" >
                    <option></option>
                   ...
                   <div class="form-group col-md-6">
                            ...
                            <input class="form-control" type="text" id="lain_lain_berhenti" name="lain_lain_berhenti" placeholder="keterangan" autocomplete="off">
                        </div>
						
					<div class="form-group col-md-4">
                    <label for="rang">Status</label>                
                    <select class="form-control" type="text" id="status_log_berhenti" name="status_log_berhenti" autocomplete="off" >
                    <option></option>
                    <option value="n">DCP</option>
						...
						<hr>
                            <button type="button" class="btn btn-success  pull-right editberhentiadd">Update</button>
...
/* $(document).ready(function() {
    ...
    <script>
		...
		$(document).on('click', '.editberhenti', function(){
                //alert($(this).data('id'));
				var id = $(this).attr('id');
				var username_fal = $(this).attr('username_fal');
				var nama_fal = $(this).attr('nama_fal');
				var kategori_pelanggan = $(this).attr('kategori_pelanggan');
				var lain_lain = $(this).attr('lain_lain');
				var status_log = $(this).attr('status_log');
				...
				$('#nama_berhenti').val(nama_fal);				
				$('#username_berhenti').val(username_fal);
				$('#kategori_pelanggan_berhenti').val(kategori_pelanggan);
				$('#lain_lain_berhenti').val(lain_lain);
				$('#status_log_berhenti').val(status_log);
				
            });
			
			// create 			
		$(document).on('click', '.editberhentiadd', function() {
    var nama_berhenti = $('#nama_berhenti').val();
    var username_berhenti = $('#username_berhenti').val();
    var kategori_pelanggan_berhenti = $('#kategori_pelanggan_berhenti').val();
    var status_log_berhenti = $('#status_log_berhenti').val();
    var lain_lain_berhenti = $('#lain_lain_berhenti').val();

    // Validasi input
    if (!status_log_berhenti) {
        Swal.fire({
            title: 'Warning!',
    ...
    // Objek data
    var value = {
        nama_berhenti: nama_berhenti,
        username_berhenti: username_berhenti,
        kategori_pelanggan_berhenti: kategori_pelanggan_berhenti,
        status_log_berhenti: status_log_berhenti,
        lain_lain_berhenti: lain_lain_berhenti
    };
    ...
    $.ajax({
        type: "POST",
        url: "../controller/action_pelangganberhenti_update.php",
        data: value,
        success: function(response) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.reload(true); // Reload halaman
                }
            });
        },
        error: function() {
