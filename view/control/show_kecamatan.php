...
if(!empty($_POST["kab_id"])){ 
    // Fetch city data based on the specific state 
    $sql = "SELECT * FROM tb_kecamatan WHERE status='y' and kd_kabkota = ".$_POST['kab_id']." ORDER BY nama_kec ASC"; 
    ...
    if(true){ 
        ...
        while($row = mysqli_fetch_assoc($query)){  
            echo '<option value="'.$row['kd_kec'].'|'.$row['kd_kabkota'].'">'.$row['nama_kec'].'</option>'; 
        } 
    }else{ 
