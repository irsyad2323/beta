$(document).ready(function () {
      ...
      success: function (response) {
        if (response.status === 200) {
          ...
          data.forEach(function (item, index) {
            ...
            const badgeStatus = item.status ? `<small class="badge badge-${statusBadges[item.status] || 'secondary'} text-uppercase">${item.status == 'follow_up' ? 'followup' : item.status}</small>` : '-';
  ...
  $('#status').on('change', function () {
    ...
    const elements = ['total_komitmen', 'deskripsi_komitmen', 'berita_acara'];
  ...
  $(document).on('click', '.editPermitStatus', function () {
    ...
    var attributes = ['permit_status_id', 'pic', 'status', 'total_komitmen', 'deskripsi_komitmen', 'keterangan'];
    ...
    $.ajax({
      url: '../../models/get_data_business_solutions_komitmen_pembayaran_for_edit.php',
      type: 'POST',
