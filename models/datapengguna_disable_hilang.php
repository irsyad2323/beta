...
if ($level_user == "ikr") {
	$query = "SELECT * FROM tbl_fal WHERE jenis_wo='INSTALASI' and status_wo='Belum Terpasang' and pic_ikr='".$acces_user_log."' ORDER BY key_fal DESC ";
		...
		$query = "SELECT * FROM tbl_disable_noconfirm WHERE kd_layanan like '".$kota."' and kategori_pelanggan IN ('Orang tidak ada + modem tidak ada','orang tidak ada + modem ada') ORDER BY key_fal DESC";
	}
	else if ($level_user == "ts") {
		$query = "SELECT * FROM tbl_disable_noconfirm WHERE kd_layanan like '".$kota."' and kategori_pelanggan='Orang tidak ada + modem tidak ada' ORDER BY key_fal DESC";
	}
