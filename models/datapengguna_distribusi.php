...
if ($level_user == "ikr") {
	$query = "SELECT * FROM tbl_distribusi WHERE status_wo='Belum Terpasang' and pic_ikr='".$acces_user_log."' ORDER BY key_odp DESC ";
	} else if ($level_user == "Admin") {
	$query = "SELECT * FROM tbl_distribusi WHERE ORDER BY key_odp DESC";
	} 
	else if ($level_user == "kapten") {
		$query = "SELECT * FROM tbl_distribusi WHERE status_wo='Belum Terpasang' ORDER BY key_odp DESC";
	}
