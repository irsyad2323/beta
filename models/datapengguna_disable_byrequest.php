...
if ($level_user == "ikr") {
	$query = "SELECT * FROM tbl_fal WHERE jenis_wo='INSTALASI' and status_wo='Belum Terpasang' and pic_ikr='".$acces_user_log."' ORDER BY key_fal DESC ";
		...
		$query = "SELECT a.*, a.status_log as status_real, c.keterangan as ket_followup FROM tbl_disable_noconfirm a 
LEFT JOIN tb_gundala b
ON a.username_fal = b.kode_user
LEFT JOIN tbl_followup c
ON a.username_fal COLLATE utf8mb4_general_ci = c.kode_user COLLATE utf8mb4_general_ci
WHERE a.kategori_pelanggan in ('By Request') GROUP BY c.kode_user ORDER BY a.key_fal DESC, c.id DESC";
	}
	else if ($level_user == "ts") {
		$query = "SELECT * FROM tbl_disable_noconfirm WHERE kd_layanan like '".$kota."' and kategori_pelanggan='By Request' ORDER BY key_fal DESC";
	}
	else if ($level_user == "psg_dcp") {
		$query = "SELECT * FROM tbl_disable_noconfirm WHERE kd_layanan like '".$kota."' and kategori_pelanggan='By Request' and pic_ikr='".$acces_user_log."' ORDER BY key_fal DESC ";
	}
