...
if ($level_user == "ikr") {
	$query = "SELECT * FROM tbl_disable WHERE status_wo='Belum Terpasang' and jenis_wo='DISMANTLE' and pic_ikr='".$acces_user_log."' ORDER BY key_fal DESC ";
	} else if ($level_user == "Admin") {
	$query = "SELECT * FROM tbl_disable WHERE  jenis_wo='DISMANTLE' ORDER BY key_fal DESC";
	} 
	else if ($level_user == "kapten") {
		$query = "SELECT * FROM tbl_disable WHERE jenis_wo='DISMANTLE' and status_wo='Belum Terpasang' ORDER BY key_fal DESC";
	}
	else if ($level_user == "ts") {
		$query = "SELECT * FROM tbl_disable WHERE jenis_wo='DISMANTLE' and tanggal_instalasi = CURDATE() ORDER BY key_fal DESC ";
	}
