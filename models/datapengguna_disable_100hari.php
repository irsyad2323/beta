...
if ($level_user == "ikr") {
	$query = "SELECT * FROM tbl_disable_noconfirm WHERE kd_layanan like '".$kota."' and status_wo='Belum Terpasang' and pic_ikr='".$acces_user_log."' ORDER BY key_fal DESC ";
	} else if ($level_user == "Admin") {
	$query = "SELECT * FROM tbl_disable_noconfirm WHERE kd_layanan like '".$kota."' and produk='Kapten Naratel' ORDER BY key_fal DESC";
	} 
	else if ($level_user == "kapten") {
		$query = "SELECT * FROM tbl_disable_noconfirm WHERE kd_layanan like '".$kota."' and kategori_pelanggan='' ORDER BY key_fal DESC";
	}
