...
$acces_user_log = $_SESSION["username"];
		...
		$query = "SELECT * FROM tbl_lain_lain WHERE kd_layanan='".$kota."' and status_wo='belum' ORDER BY key_fal DESC";
...
foreach($data as $dataz){
	...
	$data[$i]['action']='<div class="btn-group">	 
							...
							alamat_user="'.$data[$i]["alamat_user"].'"
							kelurahan="'.$data[$i]["kelurahan"].'"							
							telp_user="'.$data[$i]["telp_user"].'"
							paket="'.$data[$i]["paket"].'"
							expiration="'.$data[$i]["expiration"].'"
							jumday="'.$data[$i]["jumday"].'"
							status="'.$data[$i]["status"].'"
							tgl_jthtmpo="'.$data[$i]["tgl_jthtmpo"].'"
							tangga_jthtempo="'.$data[$i]["tangga_jthtempo"].'"
							kd_layanan="'.$data[$i]["kd_layanan"].'"
							keterangan="'.$data[$i]["keterangan"].'"
							keterangan2="'.$data[$i]["keterangan2"].'"
							janji_bayar="'.$data[$i]["janji_bayar"].'"
