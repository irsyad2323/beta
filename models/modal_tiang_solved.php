...
$query = "SELECT CONCAT(a.provinsi,'.',a.kabkota,'.',a.kecamatan,'.',a.kelurahan,'.',a.id_tiang) as id , a.* FROM tbl_instalasi_tiang a WHERE a.kd_layanan like '$kota' and a.status='Sudah Dikerjakan' and a.`status`='Sudah Dikerjakan';";
...
foreach($data as $dataz){
	...
	$data[$i]['no']=$no;
	$data[$i]['targets'] = '<label><input type="checkbox" value ="'.$data[$i]['key'].'" name = "key" class="minimal"></label>';
	...
	$data[$i]['layanan'] = '<small class="badge badge-success">Naratel</small>';
    }elseif($data[$i]['kd_layanan'] == 'pas'){
		$data[$i]['layanan'] = '<small class="badge badge-success">SBM</small>';
	}elseif($data[$i]['kd_layanan'] == 'batu'){
		$data[$i]['layanan'] = '<small class="badge badge-success">JALIBAR</small>';
	}elseif($data[$i]['kd_layanan'] == 'mlg1'){
		$data[$i]['layanan'] = '<small class="badge badge-success">JALANTRA</small>';
	}else{
		$data[$i]['layanan'] = $data[$i]['kd_layanan'];
	}
