...
if ($level_user == "ikr") {
	$query = "SELECT * FROM tbl_maintenance WHERE status_wo='Belum Terpasang' and jenis_wo='MAINTENANCE' and pic_ikr='".$acces_user_log."' ORDER BY key_fal DESC ";
	} else if ($level_user == "Admin") {
	$query = "SELECT * FROM tbl_maintenance WHERE  jenis_wo='MAINTENANCE' ORDER BY key_fal DESC";
	} 
	else if ($level_user == "kapten") {
		$query = "SELECT * FROM tbl_maintenance WHERE jenis_wo='MAINTENANCE' and status_wo in ('Belum Terpasang','Proses Pengerjaan') and kd_layanan like '".$kota."' ORDER BY key_fal DESC";
	}
	else if ($level_user == "ts") {
		$query = "SELECT * FROM tbl_maintenance WHERE jenis_wo='MAINTENANCE' and kd_layanan='".$kota."' and tanggal_instalasi = CURDATE() ORDER BY key_fal DESC ";
	}
	else if ($level_user == "super") {
		$query = "SELECT * FROM tbl_maintenance WHERE jenis_wo='MAINTENANCE' and status_wo='Belum Terpasang' ORDER BY key_fal DESC";
	}
...
foreach($data as $dataz){
    ...
    }elseif($data[$i]['status_wo'] == 'Proses Pengerjaan'){
		$data[$i]['type_status'] = '<small class="badge badge-warning">'. strtoupper($data[$i]['status_wo']).'</small>';
