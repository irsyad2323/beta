...
BETWEEN '$start_time' and '$end_time' and DATE(a.tgl_fal_datetime) = DATE('$tanggal_hari_ini') and a.kd_layanan in ('pas','pas1')
union all
...
BETWEEN '$start_time' and '$end_time' and DATE(b.tgl_date_time) = DATE('$tanggal_hari_ini') and b.kd_layanan in ('pas','pas1')
union all
...
BETWEEN '$start_time' and '$end_time' and DATE(b.tgl_sign) = DATE('$tanggal_hari_ini') and b.kd_layanan in ('pas','pas1')
union all
...
BETWEEN '$start_time' and '$end_time' and DATE(b.tgl_sign) = DATE('$tanggal_hari_ini') and b.kd_layanan_backbone in ('pas','pas1')
union all
...
BETWEEN '$start_time' and '$end_time' and DATE(c.tgl_sign) = DATE('$tanggal_hari_ini') and c.kd_layanan in ('pas','pas1')
...
<div class="alert alert-info">PAS (SBM) Proses Pengerjaan - Time Slot: <?= $time_slot ?>, Tanggal: <?= $tanggal_hari_ini ?>, Total: <?= $row_count ?> records</div>
    ...
    <table class="table table-bordered" width="100%" cellspacing="0">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><small class="badge <?= $tanggal_badge ?>"><?= date('d/m/Y H:i', strtotime($row['tgl_fal_datetime'])) ?></small></td>
                <td><?= $row['nama_fal'] ?></td>
                <td><?= $row['tlp_fal'] ?></td>
                <td><?= $row['alamat_fal'] ?>, <?= $row['kelurahan'] ?></td>
                <td><small class="badge badge-secondary"><?= $row['pic_ikr'] ?></small></td>
                <td><small class="badge badge-danger">SBM</small></td>
                <td>
                    <div class="dropdown">
