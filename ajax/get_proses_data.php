...
while ($row = mysqli_fetch_assoc($table)) {
    ...
    if ($row['jenis_wo'] == 'INSTALASI') {
        $actions .= '<a href="https://pendaftaran.kaptennaratel.com/' . $row['foto_dpn_rmh'] . '" target="_blank" class="dropdown-item">Foto Rumah</a>';
        $actions .= '<a href="' . $row['lokasi'] . '" target="_blank" class="dropdown-item">Lokasi</a>';
    }
    
    $actions .= '<a class="dropdown-item respro" data-id="' . $row['username_fal'] . '">Reschedule</a>';
    $actions .= '<a class="dropdown-item switchpic" data-id="' . $row['key_fal'] . '">Switch PIC</a>';
    $actions .= '</div></div>';
